...
import { type FunctionComponent, createContext, h } from "preact";
...
describe("web components", () => {
	...
	const Clock: FunctionComponent<{ time: string }> = ({ time }) => {
	...
	registerElement(NullProps, "x-null-props", ["size"], { shadow: "open" });
	...
	registerElement(Foo, "x-foo", [], { shadow: "open" });
	...
	registerElement(DisplayTheme, "x-display-theme", [], { shadow: "open" });

	function Parent({ children, theme = "dark" }: { children: unknown[]; theme?: string }) {
		return (
	...
	registerElement(Parent, "x-parent", ["theme"], { shadow: "open" });
